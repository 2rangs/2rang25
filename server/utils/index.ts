...
export function initSocket(event: H3Event) {
    ...
    io.on('connection', (socket: Socket) => {
        ...
        socket.on('joinRom', (payload: User) => {
            ...
            socket.broadcast.to(user.room).emit('message', formnatMessage(botName, `${user.username.split('|')[0]} has joined the chat`))
        ...
        socket.on('disconnect', () => {
            ...
            if (user) {
                ...
                io.to(user.room).emit('message', formnatMessage(botName, `${user.username.split('|')[0]} has left the chat`))
